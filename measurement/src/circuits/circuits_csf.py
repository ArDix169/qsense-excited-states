...
def find_sen_orb_dominant(tz_state, target_seniority):
    coeffs = np.abs(tz_state[2])
    dominant_idx = np.argmax(coeffs)
    SD = tz_state[0][dominant_idx]  # SD index 2, not SD index 0
    orb_occ = SD[::2] + SD[1::2]
    target = np.array(orb_occ == target_seniority, int)
    orbitals = [i for i in range(len(target)) if target[i]]
    return target, orbitals
...
#     print("DEBUG: entered get_csfs_from_dump")
...
#
#         print(f"t_vec: {t_vec}, somo: {somo}, doub_occ: {doub_occ}")
#
...
def get_csfs_from_dump(input_file, verify_states=False, verbose=True):
    ...
    if len(loaded) == 14:
        if verbose: print("Detected new dump format (14 fields)")
        (list_CSF, list_list_ia_CSF, list_list_theta_CSF, list_sym_CSF_vec,
         list_UCSF_tz, list_UCSF_smik, list_list_SOMO_UCSF_smik,
         list_list_extra_vecs, psi_GS_UCSF_smik,
         list_orb_rot, x_orbrot, Enuc, obt_spatial, tbt_spatial) = loaded
        if verbose:
            print(f"type(list_CSF): {type(list_CSF)}")
            print(f"len(list_CSF): {len(list_CSF)}")
            print(f"type(list_list_ia_CSF): {type(list_list_ia_CSF)}")
            print(f"len(list_list_ia_CSF): {len(list_list_ia_CSF)}")
            print(f"type(list_UCSF_tz): {type(list_UCSF_tz)}")
            print(f"len(list_UCSF_tz): {len(list_UCSF_tz)}")
            print(f"type(list_list_extra_vecs): {type(list_list_extra_vecs)}")
            print(f"len(list_list_extra_vecs): {len(list_list_extra_vecs)}")
    elif len(loaded) == 13:
        if verbose: print("Detected old dump format (13 fields)")
        (list_CSF, list_list_ia_CSF, list_list_theta_CSF, list_sym_CSF_vec,
         list_UCSF_tz, list_UCSF_smik, list_list_SOMO_UCSF_smik,
         psi_GS_UCSF_smik, list_orb_rot, x_orbrot, Enuc, obt_spatial, tbt_spatial) = loaded
        list_list_extra_vecs = [[] for _ in range(len(list_CSF))]
    else:
        raise ValueError(f"Unknown dump format with {len(loaded)} fields")

    n_orb = len(tbt_spatial)
    if verbose: print(f"n_orb: {n_orb}")
    ne    = sum(list_CSF[0][0][0])
    csfs  = []

    # build one CSF object per state in list_UCSF_smik
    # group structure: 1 ref + len(extra_vecs) extra states per CSF group
    ucsf_idx = 0
    for iCSF in range(len(list_CSF)):
        list_ia = list_list_ia_CSF[iCSF]
        extra_vecs = list_list_extra_vecs[iCSF]
        n_states_in_group = 1 + len(extra_vecs)
        ...
        # reference CSF — use list_CSF[iCSF] not list_UCSF_tz
        ref_state = list_CSF[iCSF]  # ← bare reference CSF

        #doub_occ, domo = find_sen_orb(ref_state, 2)
        doub_occ, domo = find_sen_orb_dominant(ref_state, 2)
        sing_occ, somo = find_sen_orb(ref_state, 1)
        t_vec = determine_t_vec(ref_state, somo)

        if verbose: print(f"CSF {iCSF}: t_vec={t_vec}, somo={somo}")

        csf = CSF(t_vec, orbitals=somo, n_orb=n_orb, ne=ne,
                  excitations=excitations, doub_occ=doub_occ)
        csfs.append(csf)
        ucsf_idx += 1

        # extra basis states — use list_UCSF_tz for these
        for j in range(1, n_states_in_group):
            actual_state = list_UCSF_tz[ucsf_idx]

            coeffs = np.abs(actual_state[2])
            dominant_idx = np.argmax(coeffs)
            SD = actual_state[0][dominant_idx]
            orb_occ = SD[::2] + SD[1::2]

            doub_occ_e = np.array(orb_occ == 2, int)  # ← correct binary array
            somo_e = [k for k in range(len(orb_occ)) if orb_occ[k] == 1]
            t_vec_e = determine_t_vec(actual_state, somo_e)

            if verbose:
                print(f"  extra state {j}: t_vec={t_vec_e}, somo={somo_e}, doub_occ={doub_occ_e}")

            csf_extra = CSF(t_vec_e, orbitals=somo_e, n_orb=n_orb, ne=ne,
                            excitations=[], doub_occ=doub_occ_e)
            csfs.append(csf_extra)
            ucsf_idx += 1
