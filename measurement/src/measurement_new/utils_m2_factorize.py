...
def group_correlated_orbitals(psi, orbitals, n_total_qubits, tol=1e-12, verify=True):
    """
    Partition `orbitals` into groups that must be kept together when factorizing.

    Being invariant under the V/W rotations (i.e. landing in S_N) does NOT imply
    being separable: an open-shell CSF spin-couples its SOMOs, so orbitals that
    no rotation touches can still be entangled with one another.  A seniority-6
    CSF, for instance, is a product of three singlet pairs -- splitting those
    pairs into single orbitals cannot reproduce the state.

    Two orbitals are correlated when the occupations they take across the
    state's support do not form a full product set: if orbital i takes n_i
    distinct values, orbital j takes n_j, and fewer than n_i * n_j joint
    patterns occur, then knowing one constrains the other.  Correlated orbitals
    are merged (union-find) and returned as a single tuple.

    psi:             full state vector (2**n_total_qubits,)
    orbitals:        iterable of qubit indices to partition
    n_total_qubits:  total number of qubits in psi

    Returns a list of tuples, sorted; orbitals that are uncorrelated with
    everything come back as 1-tuples, reproducing the previous
    one-factor-per-orbital behaviour for separable states.
    """
    orbitals = sorted(orbitals)
    if len(orbitals) < 2:
        return [(o,) for o in orbitals]

    nz = np.nonzero(np.abs(np.asarray(psi).flatten()) > tol)[0]
    if len(nz) < 2:
        # single basis state: a product state, nothing to group
        return [(o,) for o in orbitals]

    # bit of orbital q in a basis index (qubit 0 is the most significant)
    bits = {q: (nz >> (n_total_qubits - 1 - q)) & 1 for q in orbitals}

    parent = {o: o for o in orbitals}
    ...
    for ii, i in enumerate(orbitals):
        n_i = len(np.unique(bits[i]))
        if n_i < 2:
            continue                      # constant orbital: independent
        for j in orbitals[ii + 1:]:
            n_j = len(np.unique(bits[j]))
            if n_j < 2:
                continue
            n_ij = len({(a, b) for a, b in zip(bits[i], bits[j])})
            if n_ij < n_i * n_j:          # not a full product -> correlated
                union(i, j)
    ...
    blocks = sorted(tuple(sorted(g)) for g in groups.values())

    if verify:
        blocks = _merge_blocks_until_pure(psi, blocks, n_total_qubits)

    return blocks


def _block_is_pure(psi, block, n_total_qubits, tol=1e-10):
    """True when `block` is unentangled from the rest of the state.

    Reshaping the (normalized) state so `block` indexes the rows gives a matrix
    whose Schmidt rank is 1 exactly when the block factorizes out.
    """
    t = np.asarray(psi).flatten()
    nrm = np.linalg.norm(t)
    if nrm == 0:
        return True
    t = (t / nrm).reshape([2] * n_total_qubits)
    rest = [q for q in range(n_total_qubits) if q not in block]
    t = np.transpose(t, list(block) + rest).reshape(2 ** len(block), -1)
    s = np.linalg.svd(t, compute_uv=False)
    return len(s) < 2 or s[1] < tol


def _merge_blocks_until_pure(psi, blocks, n_total_qubits):
    """Merge blocks until every one factorizes out of the state.

    Pairwise correlation finds independent singlet pairs, but it cannot see
    couplings that are pairwise-uniform yet jointly constrained (parity-type
    spin couplings, which show up at seniority >= 4 with determinant counts
    that are not powers of two).  Those blocks come back impure, so merge the
    impure ones and re-test.  This terminates: in the worst case everything
    collapses into a single block, which is always pure because the full state
    is pure.
    """
    blocks = list(blocks)
    while True:
        impure = [b for b in blocks if not _block_is_pure(psi, b, n_total_qubits)]
        if not impure:
            return sorted(blocks)
        if len(impure) == 1:
            # entangled with something already deemed pure -- fall back to
            # merging it with every other non-trivial block
            others = [b for b in blocks if b not in impure]
            merged = tuple(sorted(set(impure[0]).union(*[set(b) for b in others]))) \
                if others else impure[0]
            if merged == impure[0]:
                return sorted(blocks)          # cannot do better; let caller assert
            blocks = [merged]
            continue
        merged = tuple(sorted(set().union(*[set(b) for b in impure])))
        blocks = [b for b in blocks if b not in impure] + [merged]
...
def factorize_state(basis_csr_state, S_w, S_v, S_n, state_type, correct_sign=True):
    ...
    # Handle S_N block - factorize into individual spatial orbitals, EXCEPT
    # where the orbitals are spin-coupled to each other.  S_N means "invariant
    # under the V/W rotations", which is not the same as "separable": the SOMOs
    # of an open-shell CSF are entangled even though no rotation touches them
    # (a seniority-6 CSF is a product of three singlet PAIRS, not of six
    # independent orbitals).  group_correlated_orbitals returns 1-tuples when
    # the orbitals really are separable, so this reduces to the previous
    # one-factor-per-orbital behaviour in that case.
    for _blk in group_correlated_orbitals(full_state, S_n, n_spatial_orbitals):
        factorization_dict[_blk], _ = partial_trace_einsum(
            full_state, _blk, n_spatial_orbitals
        )
