...
def project_out_seniority_symmetries(H, Nqubits, v, w):
    ...
    """
    return project_out_seniority_symmetries_prerotated(
        seniority_rotate_hamiltonian(H, Nqubits), v, w)


def seniority_rotate_hamiltonian(H, Nqubits):
    """
    Steps 1-2 of project_out_seniority_symmetries: the Clifford rotation and the
    qubit reordering.

    This depends only on H and Nqubits -- NOT on the seniority configs v, w -- so
    when tapering one Hamiltonian against many (v, w) pairs it can be computed
    once and reused.  It is also the expensive half: apply_unitary_product
    multiplies QubitOperators, and openfermion deepcopies the whole term dict on
    every multiplication.

    The map is linear in H and leaves the identity alone, so for a scalar shift c
    the rotation of (H - c) equals (rotation of H) - c.  Callers whose shift
    varies per iteration can therefore still hoist this out of the loop.
    """
    Ucliff    = seniority_solving_clifford_operator(Nqubits)
    H_rotated = apply_unitary_product(H, Ucliff)
    return group_odds_and_evens(H_rotated, Nqubits)


def project_out_seniority_symmetries_prerotated(H_rotated_reordered, v, w):
    """
    Steps 3-4 of project_out_seniority_symmetries, taking an already-rotated
    Hamiltonian from seniority_rotate_hamiltonian.  This is the only part that
    depends on the seniority configurations v and w.
    """
    return taper_hamiltonian(H_rotated_reordered, v, w, shift_to_zero=True)
