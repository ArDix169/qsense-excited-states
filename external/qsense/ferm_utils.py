...
#def op_action_tz_CSF_in_chunks(op,CSF_input,nparal=1):
#
...
#    LC_CSFs(list_CSF_output,LC_coefs)
...
def number_ferm_tz(i,lreverse = False,scal = 1.0):
    """
    Femronic number operator for mode i
    n_i = a_i^+ * a_i.
    When lreverse, 1-ni = a_i * a_i^+ is returned.
    """

    a_dagger = FermionOperator((i,1),1.0)
    if not lreverse:
        proj_op = a_dagger * hermitian_conjugated(a_dagger)
    else:
        proj_op = hermitian_conjugated(a_dagger) * a_dagger
